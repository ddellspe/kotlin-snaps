...
class KotlinSnapTest {

    @TestFixture()
    fun kotlinSnap_runStandardWithInputDoc() {
    }

    @TestFixture(
        propertyOverrides = ["$.settings.propKey.expression", "false", "$.settings.propValue.expression", "false"]
    )
    fun kotlinSnap_runStandardWithInputDocNoExpression() {
    }
}